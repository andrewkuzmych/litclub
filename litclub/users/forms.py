...
class RegisterForm(forms.Manipulator):
    def __init__(self):
        self.fields = (
        forms.TextField(field_name="username", length=30, max_length=30, is_required=True,
                        validator_list=[validators.isAlphaNumeric, validators.isLowerCase, self.isValidUsername]),
...
class PasswordChangeForm(forms.Manipulator):
    def __init__(self, user):
        ...
        forms.PasswordField(field_name="new_password1", length=30, max_length=30, is_required=True,
                            validator_list=[
                                    validators.AlwaysMatchesOtherField('new_password2', "Паролі не співпадають")]),
